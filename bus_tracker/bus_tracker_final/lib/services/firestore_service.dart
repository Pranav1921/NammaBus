...
class FirestoreService {
  ...
  //=========== NEW, CORRECTED METHOD ADDED HERE ===========
  /// Updates the conductor's profile with their operational info.
  Future<void> updateConductorOperationalInfo({
    required String uid,
    required String busNumber,
    required String driverId,
  }) {
    return _db.collection('conductors').doc(uid).set({
      'busNumber': busNumber,
      'driverId': driverId,
    }, SetOptions(merge: true)); // Use merge to avoid overwriting other profile data
  }

  /// Updates the alert status and message for a specific bus.
  Future<void> updateBusAlert(String busId, String? alertMessage) async {
    try {
      final bool isAlertActive = alertMessage != null;
      await _db.collection('buses').doc(busId).update({
        'isAlertActive': isAlertActive,
        'alertMessage': alertMessage,
      });
    } catch (e) {
      print('Error updating bus alert status: $e');
    }
  }
  //=========================================================
  ...
  Future<void> stopTracking(String busId) async {
    ...
    if (doc.exists) {
      await docRef.update({'isTracking': false, 'isAlertActive': false, 'alertMessage': null});
    }
  }
